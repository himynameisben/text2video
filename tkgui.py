...
)  # Import create_video_with_subtitles function from subclip
...
def create_subtitled_video():
    ...
    try:
        create_video_with_subtitles(srt_path, mp3_path, output_path, width, height, font_size)
        messagebox.showinfo(
...
def set_resolution(width, height, font_size):
...
button_cancel_settings = tk.Button(
    settings_frame, text="Cancel", command=cancel_settings
)
...
button_select_folder = tk.Button(
    tts_frame, text="Select TTS Output Folder", command=select_folder
)
...
resolution_frame = tk.LabelFrame(subclip_frame, text="Resolution Settings", padx=10, pady=10)
...
button_preset_1080p = tk.Button(preset_frame, text="1080p", command=lambda: set_resolution(1920, 1080, 28))
button_preset_1080p.grid(row=0, column=0, padx=5, pady=5)

button_preset_720p = tk.Button(preset_frame, text="720p", command=lambda: set_resolution(1280, 720, 24))
button_preset_720p.grid(row=0, column=1, padx=5, pady=5)

button_preset_640p = tk.Button(preset_frame, text="640p", command=lambda: set_resolution(640, 360, 20))
...
button_create_video = tk.Button(
    subclip_frame, text="Create Video with Subtitles", command=create_subtitled_video
)
button_create_video.pack(pady=10)

button_back_to_main = tk.Button(
    subclip_frame, text="Back to Main", command=show_main_page
)
