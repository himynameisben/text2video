...
def make_subtitle_clip(subs):
    def generator(txt):
        ...
        return TextClip(txt, font='Arial', fontsize=48, color='white', size=(VIDEO_WIDTH, None), method='caption', align='center')
...
def create_video_with_subtitles(srt_file, mp3_file, output_file):
    ...
    video_clip = ColorClip(size=(VIDEO_WIDTH, VIDEO_HEIGHT), color=(0, 0, 0), duration=audio_clip.duration)
    ...
    final_clip = CompositeVideoClip([video_clip, subtitle_clip.set_pos(('center', 'center'))])
...
if __name__ == '__main__':
    # Example usage
    create_video_with_subtitles('sample/ok_google.srt', 'sample/ok_google.mp3', 'sample/ok_google.mp4')
